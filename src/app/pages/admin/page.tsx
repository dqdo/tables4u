...
export default function Home() {
    ...
    const [resReservationList, setResReservationList] = useState<{ restaurant_id: number; confirmation_number: number; email: string }[]>([]);
    const [searchRID, setSearchRID] = useState(0)
    ...
    function getReservationByID() {
        instance.post('/get_restaurant_reservations', { restaurant_id: searchRID })
            .then(function (response) {
                const { statusCode, result, error } = response.data;

                if (statusCode === 200) {

                    const reservationInfo = result.reservations.map((reservation: { confirmation_number: any; email: any }) => ({
                        restaurant_id: result.restaurant_id,
                        confirmation_number: reservation.confirmation_number,
                        email: reservation.email
                    }));
                    setResReservationList(reservationInfo);
                    console.log(resReservationList)
                } else {
        ...
        <div>
            ...
            <div className="deleteReservationContainer">
                ...
                <div className="availabilityReportContainer">
                    ...
                    <button className="button" onClick={determineUtilization}>Generate Report</button>
                ...
                <div className="searchForReservation">
                    Search Reservations by ID: <input
                        type="text"
                        className="text"
                        value={searchRID}
                        onChange={(e) => {
                            const value = e.target.value;
                            setSearchRID(Number(value));
                        }}
                    />
                    <button className="button" onClick={getReservationByID}> Search </button>
                </div>

                <label className="reservationSearchList">
                    {resReservationList.map((res) => (
                        <div key={res.restaurant_id}>
                            ID: {res.restaurant_id} | Confirmation Code: {res.confirmation_number} | Email: {res.email}
                        </div>
                    ))}
                </label>
